...
def main():
    ...
    # Adjust path relative to the script's location in the cecli repo
    litellm_path = script_dir.parent / "../litellm/model_prices_and_context_window.json"
    cecli_path = script_dir / "../cecli/resources/model-metadata.json"

    if not litellm_path.exists():
        print(f"Error: LiteLLM metadata file not found at {litellm_path}")
        return

    if not cecli_path.exists():
        print(f"Error: cecli metadata file not found at {cecli_path}")
        return
    ...
    try:
        # Use json5 for the cecli metadata file as it might contain comments
        with open(cecli_path, "r") as f:
            cecli_data = json5.load(f)
    except json.JSONDecodeError as e:
        print(f"Error decoding JSON from {cecli_path}: {e}")
        return
    except Exception as e:
        print(f"Error reading {cecli_path}: {e}")
        return

    litellm_keys = set(litellm_data.keys())
    cecli_keys = set(cecli_data.keys())

    common_keys = sorted(list(litellm_keys.intersection(cecli_keys)))
    removed_count = 0
    ...
    if common_keys:
        ...
        for key in common_keys:
            ...
            cecli_entry_copy = cecli_entry.copy()
            dummy_key = "zzzdummykey"
            litellm_entry_copy[dummy_key] = True
            cecli_entry_copy[dummy_key] = True

            litellm_json_lines = json.dumps(
                litellm_entry_copy, indent=4, sort_keys=True
            ).splitlines()
            cecli_json_lines = json.dumps(cecli_entry_copy, indent=4, sort_keys=True).splitlines()
            ...
            cecli_json_filtered = [line for line in cecli_json_lines if dummy_key not in line]

            diff = difflib.unified_diff(
                cecli_json_filtered,
                litellm_json_filtered,
                fromfile=f"{key} (cecli)",
                tofile=f"{key} (litellm)",
                lineterm="",
                n=max(len(litellm_json_filtered), len(cecli_json_filtered)),  # Show all lines
            )
            ...
            # Ask user if they want to remove the entry from cecli's metadata
            response = (
                input(f"Remove '{key}' from cecli/resources/model-metadata.json? (y/N): ")
                .strip()
                .lower()
            )
            if response == "y":
                # Perform surgical removal from the text file
                if remove_block_surgically(cecli_path, key):
                    removed_count += 1
                    # Optional: Also remove from the in-memory dict if needed later,
                    # but it's not strictly necessary if we reload or finish now.
                    # if key in cecli_data: del cecli_data[key]
                else:
                    print(f"Failed to remove '{key}' block surgically.")
                    # Key might still be in cecli_data if removal failed
            else:
                print(f"Keeping '{key}'.")
            print("-" * 40 + "\n")  # Separator for the next model
